...
public abstract class Report {
   ...
   */
  public Report() {
    this.lastOutputSuffix = 0; // 最后输出后缀
    this.outputInterval = -1; // 输出间隙
    this.warmupIDs = null;
    ...
    if (settings.contains(WARMUP_S)) { // 预热周期
      this.warmupTime = settings.getInt(WARMUP_S);
    } else {
      this.warmupTime = 0;
    }

    if (settings.contains(PRECISION_SETTING)) { // 小数位数
    ...
    if (settings.contains(OUTPUT_SETTING)) {//输出文件路径
