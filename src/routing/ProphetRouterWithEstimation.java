...
public class ProphetRouterWithEstimation extends ActiveRouter {
  ...
  private boolean updateIET(DTNHost host) {
    ...
    if (meetings.containsKey(host)) {
      ...
      nrofSamples++;
      meanIET =
          (((double) nrofSamples - 1) / (double) nrofSamples) * meanIET
  ...
  private void updateParams() {
    ...
    while (cond == false) {
      ...
      pavg = (1 / (b * meanIET)) * (1 - zeta * (1 - pstable)) * (1 - Math.exp(-b * meanIET));
  ...
  private class TupleComparator implements Comparator<Tuple<Message, Connection>> {
    ...
    public int compare(Tuple<Message, Connection> tuple1, Tuple<Message, Connection> tuple2) {
      ...
      double p1 =
          ((ProphetRouterWithEstimation) tuple1.getValue().getOtherNode(getHost()).getRouter())
              .getPredFor(tuple1.getKey().getTo());
      // -"- tuple2...
      double p2 =
          ((ProphetRouterWithEstimation) tuple2.getValue().getOtherNode(getHost()).getRouter())
  ...
  public RoutingInfo getRoutingInfo() {
    ...
    ri.addMoreInfo(
        new RoutingInfo(String.format("meanIET: %f\t from %d samples", meanIET, nrofSamples)));
