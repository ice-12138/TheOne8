...
public abstract class ActiveRouter extends MessageRouter {
  ...
  public Message messageTransferred(String id, DTNHost from) {
    ...
    if (m.getTo() == getHost() && m.getResponseSize() > 0) {
      ...
      Message res =
          new Message(
              this.getHost(), m.getFrom(), RESPONSE_PREFIX + m.getId(), m.getResponseSize());
  ...
  public void update() {
    ...
    for (int i = 0; i < this.sendingConnections.size(); ) { // 检查所有连接是否在使用，删除未在使用的连接
      Connection con = sendingConnections.get(i);
      boolean removeCurrent = false;
      /* finalize ready transfers */
      if (con.isMessageTransferred()) {
        if (con.getMessage() != null) {
          transferDone(con); // 传输完成
          con.finalizeTransfer(); // 结束传输，调用该方法后再该链接上就不得再进行传输
        } /* else: some other entity aborted transfer */
        removeCurrent = true;
      }
      /* remove connections that have gone down */
      else if (!con.isUp()) { // 如果连接不正常
        if (con.getMessage() != null) {
          transferAborted(con); // 非正常终止传输
          con.abortTransfer();
        }
        removeCurrent = true;
      }
    ...
    if (SimClock.getTime() - lastTtlCheck >= TTL_CHECK_INTERVAL && sendingConnections.isEmpty()) {
      dropExpiredMessages();
  ...
  public RoutingInfo getRoutingInfo() {
    ...
    if (energy != null) {
      top.addMoreInfo(
          new RoutingInfo("Energy level: " + String.format("%.2f", energy.getEnergy())));
    }
    return top;
  }
}
