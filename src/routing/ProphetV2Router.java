...
public class ProphetV2Router extends ActiveRouter {
  /** delivery predictability initialization constant 交付可预见性初始化常数 */
  public static final double PEncMax = 0.5;

  /** typical interconnection time in seconds 典型互连时间(以秒为单位) */
  public static final double I_TYP = 1800;

  /** delivery predictability transitivity scaling constant default value 交付可预测性传递性缩放常数默认值 */
  public static final double DEFAULT_BETA = 0.9;

  /** delivery predictability aging constant 交货可预见性老化常数 */
  ...
  private void updateTransitivePreds(DTNHost host) {
    ...
    for (Map.Entry<DTNHost, Double> e : othersPreds.entrySet()) {
      ...
      if (pNew > pOld) preds.put(e.getKey(), pNew);
    }
  }
  ...
  private class TupleComparator implements Comparator<Tuple<Message, Connection>> {
    ...
    public int compare(Tuple<Message, Connection> tuple1, Tuple<Message, Connection> tuple2) {
      ...
      double p1 =
          ((ProphetV2Router) tuple1.getValue().getOtherNode(getHost()).getRouter())
              .getPredFor(tuple1.getKey().getTo());
      // -"- tuple2...
      double p2 =
          ((ProphetV2Router) tuple2.getValue().getOtherNode(getHost()).getRouter())
