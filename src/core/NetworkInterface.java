...
public abstract class NetworkInterface implements ModuleCommunicationListener {
  ...
  public void setHost(DTNHost host) {
    ...
    if (!comBus.containsProperty(SCAN_INTERVAL_ID) && !comBus.containsProperty(RANGE_ID)) {
    ...
    if (transmitRange > 0) {
      optimizer =
          ConnectivityGrid.ConnectivityGridFactory(this.interfacetype.hashCode(), transmitRange);
  ...
  public boolean isActive() {
    ...
    if (active && host.getComBus().getDouble(EnergyModel.ENERGY_VALUE_ID, 1) <= 0) {
