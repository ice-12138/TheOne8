...
/**
 * Tests for EpidemicRouter and, due the simple nature of Epidemic router, also ActiveRouter in
 * general.
 */
public class EpidemicRouterTest extends AbstractRouterTest {
  ...
  /**
   * try disconnecting on the same update interval when a transfer should be finished -> should not
   * cause abort (anymore)
   */
  ...
  /**
   * Tests creating a new message when the message buffer is full and the message that should be
   * removed is the message being sent
   */
  ...
  public void testResponse() {
    ...
    assertTrue(mc.next());
    assertEquals(mc.TYPE_CREATE, mc.getLastType());
    assertEquals(routing.ActiveRouter.RESPONSE_PREFIX + msgId1, mc.getLastMsg().getId());
    assertEquals(h3, mc.getLastFrom());
    assertEquals(h1, mc.getLastTo());
   ...
   *
   * @param withDestination If true, the other node is the final destination of the messages, if
   *     false, the other node is not the final dst
